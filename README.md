...
- Variables are constructed as:
  ...
  - Stock market: SP500 (daily), aggregated to monthly by "last observation", YoY log growth : this was our first try but data available only as of 2016.
    so we switched to the SPASTT01USM661N of the FRED db.
- Sample window (monthly): 1963m1–2023m6 (configurable).
...
2) Priors (numerical hyperparameters):
...
- Sign/zero restrictions from Table 1 are included, with an optional check.

### Outputs
- Posterior draws are saved to `output/mcmc/posterior_draws.mat`.
- IRFs are saved to `output/irf/irf_results.mat`.
